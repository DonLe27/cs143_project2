...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    new Iterator[Row] {
      val dhrIter = DiskHashedRelation(input, keyGenerator).getIterator()
      var curIter: Iterator[Row] = null
      ...
      private def fetchNextPartition(): Boolean  = {
        /* IMPLEMENT THIS METHOD */
        if (dhrIter.hasNext) {
          curIter = CS143Utils.generateCachingIterator(projectList, child.output) (dhrIter.next.getData())
          true
        }
        else {
          false
